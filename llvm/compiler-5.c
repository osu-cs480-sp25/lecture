#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h> // Include standard boolean type header for true/false values
...
int main()
{
    ...
    */
    ...
    LLVMModuleRef module = LLVMModuleCreateWithName(
        "lecture.code.5" // Name of the module
    );
    ...
    LLVMValueRef assignment = assign_to_variable("a", constant4, symbols, builder); // Assign 4.0 to variable 'a'
    
    LLVMValueRef variable_a =  LLVMBuildLoad2(
        builder, /* The builder to use */
        LLVMFloatType(), /* The type of the variable */
        hash_get(symbols, "a"), /* The variable to load */
        "a" /* The name of the variable */
    );

    LLVMBuildRet( // Build a return instruction
         builder, // The builder to use for generating instructions
         variable_a // The value to return (the result of the arithmetic expression)
    );
