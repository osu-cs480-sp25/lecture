...
)
{
    ...
    if (hash_contains(symbols, name)) {
        ...
        variable = declare_variable(name, false, builder); // Declare the variable using the name and builder
...
int main()
{
    ...
    */
    ...
    LLVMModuleRef module = LLVMModuleCreateWithName(
        "lecture.code.7" // Name of the module
    );
    ...
    LLVMValueRef variable_a = assign_to_variable("a", sum, symbols, builder); // Assign 4.0 to variable 'a'
    
    LLVMValueRef variable_b = assign_to_variable("b", variable_a, symbols, builder); // Assign 4.0 to variable 'b'

    LLVMBuildRet( // Build a return instruction
         builder, // The builder to use for generating instructions
         variable_b // The value to return (the result of the arithmetic expression)
    );
